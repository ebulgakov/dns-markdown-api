...
var clerkClient = new Proxy(clerkClientSingleton, {
  get(_target, property) {
    ...
    const client = (0, import_backend.createClerkClient)({
      ...env,
      userAgent: `${"@clerk/express"}@${"1.7.67"}`
    });
    if (env.secretKey) {
...
`);
var satelliteAndMissingProxyUrlAndDomain =
...
var authenticateRequest = opts => {
  ...
  const isSatellite = (0, import_utils2.handleValueOrFn)(
    options?.isSatellite,
    clerkRequest.clerkUrl,
    env.isSatellite
  );
  const domain =
    (0, import_utils2.handleValueOrFn)(options?.domain, clerkRequest.clerkUrl) || env.domain;
  ...
  if (
    isSatellite &&
    !(0, import_proxy.isHttpOrHttps)(signInUrl) &&
    (0, import_keys.isDevelopmentFromSecretKey)(secretKey || "")
  ) {
...
var absoluteProxyUrl = (relativeOrAbsoluteUrl, baseUrl) => {
  if (
    ...
    !(0, import_proxy.isValidProxyUrl)(relativeOrAbsoluteUrl) ||
    !(0, import_proxy.isProxyUrlRelative)(relativeOrAbsoluteUrl)
  ) {
...
var getAuth = (req, options) => {
  ...
  return (0, import_internal2.getAuthObjectForAcceptedToken)({
    authObject,
