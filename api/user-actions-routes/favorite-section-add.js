...
async function favoriteAddHandler(req, res, next) {
    try {
        ...
        const user = await user_1.User.findOneAndUpdate({ userId: userId.trim() }, { $addToSet: { favoriteSections: title.trim() } }, { new: true, runValidators: true }).exec();
        if (!user)
...
exports.default = favoriteAddHandler;
