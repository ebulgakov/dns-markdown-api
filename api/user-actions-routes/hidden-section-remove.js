...
async function hiddenRemoveHandler(req, res, next) {
    try {
        ...
        const user = await user_1.User.findOneAndUpdate({ userId: userId.trim() }, { $pull: { hiddenSections: title.trim() } }, { new: true, runValidators: true }).exec();
        if (!user)
...
exports.default = hiddenRemoveHandler;
