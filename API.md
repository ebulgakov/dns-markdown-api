...
- **Логика**:
  ...
  - Если рассчитанный `predictionDate` оказывается в прошлом (товар «просрочил» ожидаемое изменение), товар из ответа исключается — показываются только прогнозы на будущее.
- **Ответы**:
  - `200` — массив объектов `{ item: Goods, lastUpdateDate: string, predictionDate: string }`, отсортированный по возрастанию `predictionDate` (сначала товары с ближайшим прогнозом). `lastUpdateDate` — дата последнего зафиксированного изменения цены.
  - `400` — `"city is required"`.
