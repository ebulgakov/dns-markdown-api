...
- **Назначение**: получить прогноз даты следующего изменения цены для каждого товара в текущем прайс-листе города, на основе медианного интервала между историческими изменениями цены (`AnalysisData`).
- **Авторизация**: Clerk + `X-Internal-API-Secret`.
- **Параметры query**:
  - `city` (string, required): город.
- **Логика**:
  - Для каждого товара берётся вся история его цены из `AnalysisData` (по `link`/`city`), отсортированная по дате, и из неё выделяются даты фактических изменений цены — записи, где `price` отличается от цены в предыдущей записи (в любую сторону: важен сам факт изменения, а не направление; записи без изменения цены, например только изменение прибыли, не учитываются).
  - Учитываются только «полные» интервалы — между самими изменениями. Промежутки «дата добавления товара в каталог → первое изменение» и «последнее изменение → сегодня» интервалами не считаются.
  - Сначала для всего города считается общий эталонный интервал — медиана всех интервалов между изменениями по всем товарам сразу. Он используется как для отсечения аномалий (см. ниже), так и как запасной прогноз для товаров без собственной истории.
  - Аномалии: интервал между двумя соседними изменениями конкретного товара считается аномальным (например, несколько изменений цены в течение нескольких дней подряд), если он меньше 20% от общего эталонного интервала по городу. Такие интервалы не участвуют в расчёте медианы товара, а более раннее изменение из пары отбрасывается — считается, что «настоящим» осталось последнее по времени изменение из этой пачки. Аномалия проверяется только в сторону слишком короткого интервала — товары с естественно редкими изменениями (интервал заметно больше эталонного) аномалией не считаются.
  - Если после отбраковки аномалий у товара остался хотя бы один интервал, `predictionDate` = дата последнего изменения + медиана его собственных интервалов.
  - Если у товара нет ни одного собственного интервала (менее 2 зафиксированных изменений цены, либо все интервалы оказались аномальными), используется общий эталонный интервал по городу; `predictionDate` считается от последней известной даты — это дата последнего изменения, если оно было хотя бы одно, иначе дата самой ранней записи товара в `AnalysisData`. Если по всему городу нет ни одного интервала, товар исключается из ответа.
  - Если рассчитанный `predictionDate` оказывается в прошлом (товар «просрочил» ожидаемое изменение), товар из ответа не исключается, но помечается флагом `expired: true`; для прогнозов на будущее `expired: false`.
- **Ответы**:
  - `200` — массив объектов `{ item: Goods, lastUpdateDate: string, predictionDate: string, expired: boolean }`, отсортированный по возрастанию `predictionDate` (сначала товары с ближайшим/просроченным прогнозом). `lastUpdateDate` — дата последнего зафиксированного изменения цены; `expired` — прогноз уже в прошлом.
  - `400` — `"city is required"`.
