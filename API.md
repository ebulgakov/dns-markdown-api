...
- **Назначение**: получить прогноз даты следующего изменения цены для каждого товара в текущем прайс-листе города, на основе среднего интервала между историческими изменениями цены (`AnalysisData`).
- **Авторизация**: Clerk + `X-Internal-API-Secret`.
- **Параметры query**:
  - `city` (string, required): город.
- **Логика**:
  ...
  - Средний интервал считается между датами этих изменений, и `predictionDate` = дата последнего изменения + средний интервал.
  - Товары, у которых зафиксировано менее 2 изменений цены (нет ни одного полного интервала для расчёта), из ответа исключаются.
