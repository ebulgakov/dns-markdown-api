...
- **Назначение**: получить прогноз даты следующего понижения цены для каждого товара в текущем прайс-листе города, на основе среднего интервала между историческими понижениями цены (`AnalysisData`).
- **Авторизация**: Clerk + `X-Internal-API-Secret`.
- **Параметры query**:
  - `city` (string, required): город.
- **Логика**:
  - Для каждого товара берётся вся история его цены из `AnalysisData` (по `link`/`city`), отсортированная по дате, и из неё выделяются только фактические понижения — записи, где `price` меньше цены в предыдущей записи (повышения и записи без изменения цены не учитываются).
  - Учитываются только «полные» интервалы — между самими понижениями. Промежутки «дата добавления товара в каталог → первое понижение» и «последнее понижение → сегодня» интервалами не считаются.
  - Средний интервал считается между датами этих понижений, и `predictionDate` = дата последнего понижения + средний интервал.
  - Товары, у которых зафиксировано менее 2 понижений цены (нет ни одного полного интервала для расчёта), из ответа исключаются.
  - Если рассчитанный `predictionDate` оказывается в прошлом (товар «просрочил» ожидаемое понижение), товар из ответа исключается — показываются только прогнозы на будущее.
- **Ответы**:
  - `200` — массив объектов `{ item: Goods, lastUpdateDate: string, predictionDate: string }`, отсортированный по возрастанию `predictionDate` (сначала товары с ближайшим прогнозом). `lastUpdateDate` — дата последнего зафиксированного понижения цены.
  - `400` — `"city is required"`.
