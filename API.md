...
- **Логика**:
  ...
  - Товары, у которых зафиксировано менее 2 понижений цены (недостаточно данных для расчёта интервала), из ответа исключаются.
  - `predictionDate` может быть в прошлом (если товар давно не дешевел, а исторический интервал между понижениями короткий) — это осознанно не ограничивается текущей датой, чтобы такие товары оказывались в начале списка.
- **Ответы**:
